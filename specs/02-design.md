...
| 009 | Mapa: `fit_map_boundaries: false` y capa `streets` | `satellite` con auto-encuadre (los valores por defecto) | Con 0 publicaciones el auto-encuadre no tiene a qué ajustarse e ignora el centro configurado. Además `streets` sirve más que satélite para ubicar barrios y veredas. Todas las capas son teselas de Mapbox: el cliente alojado trae la clave de Ushahidi; **un cliente propio necesitaría clave propia**. |
